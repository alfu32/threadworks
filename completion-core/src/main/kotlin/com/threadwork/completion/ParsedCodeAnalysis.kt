...
internal class ParsedCodeAnalysis(
    ...
    private val runtime: List<SourceSymbols> = emptyList(),
) : CodeAnalysis {
    private val sources = listOf(active) + companions + runtime
    private val allBindings = sources.flatMap { it.bindings }
    private val runtimeIds = runtime.map { it.nodeId }.toSet()
    private val runtimeGlobals = runtime.flatMap { unit ->
    ...
    override fun completions(): AnalysisResult<List<CompletionSuggestion>> {
        ...
        val visible = active.visible(cursor) + runtimeGlobals
        return AnalysisResult.Available(visible.distinctBy { it.name }.filter { prefix.isBlank() || it.name.startsWith(prefix, true) }.map {
            val generated = it.location.nodeId in runtimeIds
            CompletionSuggestion(it.name, it.name,
                if (generated) CompletionSuggestionKind.CompilerSymbol else CompletionSuggestionKind.UserSymbol,
                if (generated) it.header else "${it.kind.name.lowercase()} declared in this node" +
                    it.typeName.takeIf(String::isNotBlank)?.let { name -> ": $name" }.orEmpty(),
                listOf(it.header, it.documentation).filter(String::isNotBlank).joinToString("\n"))
        })
    }
    ...
    private fun binding(name: String, offset: Int, source: SourceSymbols = active): SourceBinding? =
        source.visible(offset).firstOrNull { it.name == name }
            ...
            ?: runtimeGlobals.firstOrNull { it.name == name }
    ...
    private fun bindingType(binding: SourceBinding, depth: Int): CodeType {
        ...
        val source = sources.first { it.nodeId == binding.location.nodeId }
    ...
    private fun infer(expression: String, offset: Int, depth: Int = 0, source: SourceSymbols = active): CodeType {
        ...
        binding(expr, offset, source)?.let {
            val type = bindingType(it, depth + 1)
            if (type != CodeType.Unknown || it.location.nodeId !in runtimeIds) return type
        }
    ...
    override fun members(expression: String, offset: Int): AnalysisResult<List<CodeMember>> =
        AnalysisResult.Available(membersOf(expression, offset).map { member ->
            if (member.declaration?.nodeId in runtimeIds) member.copy(declaration = null) else member
        })
    ...
    private fun membersOf(expression: String, offset: Int, depth: Int = 0, source: SourceSymbols = active): List<CodeMember> {
        ...
        val modeled = if (local == null || local.location.nodeId in runtimeIds) compiler.symbols.firstOrNull { it.name == expression.trim() } else null
        if (modeled != null && modeled.kind in setOf(CompilerCodeSymbolKind.InputBuffer, CompilerCodeSymbolKind.OutputBuffer, CompilerCodeSymbolKind.ServiceInstance, CompilerCodeSymbolKind.RuntimeSymbol) &&
            (modeled.kind != CompilerCodeSymbolKind.RuntimeSymbol || modeled.members.isNotEmpty())) {
        ...
        val members = allBindings.filter { it.ownerType == target }.map {
            CodeMember(it.name.removePrefix("$"), if (it.callable) CodeMemberKind.Method else CodeMemberKind.Field,
                bindingType(it, depth + 1), it.header, it.documentation, declaration = it.location)
        }
    ...
    override fun definition(offset: Int): AnalysisResult<CodeLocation?> =
        AnalysisResult.Available(resolvedLocation(offset)?.takeUnless { it.nodeId in runtimeIds })

    private fun resolvedLocation(offset: Int): CodeLocation? {
        active.bindings.firstOrNull { offset in it.location.range }?.let { return it.location }
    ...
    override fun hover(offset: Int): AnalysisResult<CodeHoverInfo?> {
        ...
        val location = resolvedLocation(offset)
        val binding = location?.let { target -> allBindings.firstOrNull { it.location == target } }
        if (binding != null) {
            val displayType = binding.typeName.ifBlank { bindingType(binding, 0).typename() }
            ...
            return AnalysisResult.Available(CodeHoverInfo(title, listOf(extra, binding.documentation).filter(String::isNotBlank).joinToString("\n")))
        }
        ...
        if (compilerSymbol != null) {
            ...
            return AnalysisResult.Available(CodeHoverInfo(
                compilerSymbol.typeName.takeIf { '(' in it } ?: compilerSymbol.name,
